...
`PolicyReplayEngine` reads a verified evidence envelope and selects only recorded `TOOL_CALL_OBSERVED` events with fixed target tool IDs. It does not inspect raw trace data and does not launch a VM. For each relevant event it correlates the observed and blocked records by their fixed action ID and retains both references, the recorded decision, task-authorization boolean, canary-presence boolean, and deterministic decision under the requested policy. Verification recomputes the closed replay record from the supplied source envelope.
...
Policy replay answers only how the changed policy treats the recorded action. Same-policy reruns are rejected. It does not assert that a different policy would have changed prior model reasoning, and it does not re-execute any guest/model/tool effect.
