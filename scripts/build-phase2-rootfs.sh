...
BUILD_DIR="$(mktemp -d /tmp/crumple-phase2-rootfs.XXXXXX)"
IMAGE_TMP="$(mktemp /tmp/crumple-phase2-image.XXXXXX)"
OUTPUT="$CACHE/guest/rootfs-phase2.ext4"
...
gcc -Os -static -Wall -Wextra -Werror -o "$CACHE/guest/crumple-phase2-init" "$ROOT/guest/phase2_init.c"
...
install -m 0755 "$CACHE/guest/crumple-phase2-init" "$BUILD_DIR/sbin/crumple-phase2-init"
...
E2FSPROGS_FAKE_TIME="$SOURCE_DATE_EPOCH" mke2fs -q -t ext4 -F -d "$BUILD_DIR" \
  -L CRUMPLE_P2 -U 6372756d-706c-6500-0000-000000000002 \
  -E hash_seed=6372756d-706c-6500-0000-000000000002,lazy_itable_init=0,lazy_journal_init=0 \
  "$IMAGE_TMP"
...
for path in /dev /proc /run /sbin /sys /workspace /opt /opt/codex /opt/codex/bin /opt/codex/codex-resources /opt/codex/codex-path /opt/crumple /opt/crumple/skills /opt/crumple/skills/prompt-injection-observer \
  /sbin/crumple-phase2-init /sbin/crumple-http-forwarder /sbin/crumple-mcp-proxy /opt/codex/bin/codex /opt/codex/codex-resources/bwrap /opt/codex/codex-path/rg /opt/crumple/skills/prompt-injection-observer/SKILL.md; do
...
e2fsck -fn "$OUTPUT"
sha256sum "$CACHE/guest/crumple-phase2-init" "$CACHE/guest/crumple-http-forwarder" "$CACHE/guest/crumple-mcp-proxy" "$OUTPUT"
